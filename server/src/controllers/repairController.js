...
import { DeliveryJobStatus, RepairStatus } from '../utils/enums.js';
...
export async function updateRepairStatus(req, res) {
  ...
  if (currentStatus === RepairStatus.IN_PROGRESS) {
    const deliveryJob = await (await import('../models/DeliveryJob.js')).default
      .findOne({ repairRequestId: repair._id });
